...
      res.on("end", () => {
        console.log("Supabase status:", res.statusCode, "data:", data.substring(0, 200));
        resolve(data || "{}");
      });
    });
    req.on("error", reject);
...
        console.log("SAVE appelé, clients:", data.clients ? data.clients.length : 0);
        // Upsert au lieu de PATCH
